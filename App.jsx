...
export default function App(){
  ...
  const GameSectionMobile = () => (
    <div className="surface player-card" style={{marginTop:12}}>
      ...
      <div className="meta" style={{marginTop:6}}>Totals — BI <b className="mono">A${totals.buyInSum.toFixed(2)}</b> · CO <b className="mono">A${totals.cashAdjSum.toFixed(2)}</b> · Diff <b className="mono">{totals.diff.toFixed(2)}</b></div>
      <div className="toolbar" style={{justifyContent:'flex-end', marginTop:8}}>
        <button className="btn success" onClick={saveGameToHistory}>End & Save</button>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="topbar">
        ...
        <div className="top-tabs">{['game','history','ledgers','profiles'].map(id=>(
          <button key={id} className={"tabbtn"+(tab===id?" active":"")} onClick={()=>setTab(id)}>{id[0].toUpperCase()+id.slice(1)}</button>
        ))}</div>
      </div>
      <div className={"drawer-backdrop"+(drawerOpen?" open":"")} onClick={()=>setDrawerOpen(false)} />
      <div className={"drawer"+(drawerOpen?" open":"")}>
        <strong>Navigate</strong>
        <div className="navgroup">{['game','history','ledgers','profiles'].map(id=>(
          <button key={id} className={"tabbtn"+(tab===id?" active":"")} onClick={()=>{setTab(id); setDrawerOpen(False);}}>{id[0].toUpperCase()+id.slice(1)}</button>
        ))}</div>
      </div>
      ...
      <div className="container">
        ...
        {tab==='ledgers' && <div className="surface" style={{marginTop:12}}><div className="meta">Ledgers coming soon…</div></div>}
        {tab==='profiles' && <div className="surface" style={{marginTop:12}}><div className="meta">Profiles coming soon…</div></div>}
        <div className="meta" style={{marginTop:10}}>Tip: Use ☰ to switch sections on mobile. Desktop keeps tables.</div>
      </div>
    </div>
  );
}
