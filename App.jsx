...
export default function App(){
  ...
  // --- Host Lock state ---
  const [hostLock,setHostLock] = useState({ active:false, by:null, until:null, at:null }); // UI only; source of truth is server
  ...
  async function apiLockSeason(locked){
    // POST /api/season/lock  {seasonId, locked}
    const res = await fetch(`${API_BASE}/api/season/lock`, {
      method: "POST",
      headers: { "Content-Type":"application/json" },
      body: JSON.stringify({ seasonId: SEASON_ID, locked })
    });
    if(!res.ok){
      const msg = await res.text();
      throw new Error(msg || "Failed to toggle host lock");
    }
    const doc = await res.json().catch(()=>null);
    if (doc) hydrateFromDoc(doc);
    else setHostLock(s=>({ ...s, active: locked }));
  }
  ...
  function hydrateFromDoc(doc){
    ...
    // lock shape tolerant: doc.lock.active | doc.locked | doc.lock?.locked
    ...
    const t = setInterval(async()=>{
      try{
        ...
        if((doc.version||0)!==cloudVersion){
          ...
          // even if version same, lock may auto-unlock server-side (next day Brisbane)
          const lock = doc && (doc.lock || {});
          const active = !!(lock.active ?? lock.locked ?? doc.locked ?? false);
          if(active !== hostLock.active) hydrateFromDoc(doc);
        }
      }catch(e){ /* ignore transient errors */ }
    }, 10000);
    return ()=>clearInterval(t);
  },[cloudVersion, hostLock.active]);
      ...
      <div className="container">
        ...
        <Section>
          ...
          <table className="table">
            ...
            <tbody>
              {players.map(p => (
                <PlayerRow key={p.id} p={p} onChange={updatePlayer} buyInAmount={buyInAmount} />
              ))}
            </tbody>
            <tfoot>
              <tr>
