...
/**
 * PocketPoker App.jsx — Full UI + Cloud features
 * - Equal-split settlement (default), deterministic order
 * - Prize-from-pot: subtract fixed amount from each player; split pooled amount to top winner(s)
 * - Cloud season storage with Upstash-backed API routes (get/append/delete/lock/mark-payment/profile-upsert)
 * - Host lock with read-only overlay
 * - Per-transfer "Paid?" toggle; winner chips show received/needed
 * - Profiles (PayID + avatar) used in History and Transfers
 * - CSV export for Players and Transfers
 * - Validation + override mismatch
 */
...
/**
 * Adjust players for prize-from-pot:
 * - Subtract prizeAmount from everyone's cashOutAdj
 * - Pool = prizeAmount * N
 * - Distribute pool equally to top winner(s) (ties share)
 * @param base list {name, buyIns, buyInTotal, baseCash}
 * @param buyInAmount number
 * @param prizeAmount number
 * @returns adjusted list with {cashOutAdj, prize, netAdj}
 */
